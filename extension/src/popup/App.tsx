...
type PopupView = "main" | AuthMode;

function isCollectedRecord(record: CnkiRecord) {
  return Boolean(record.detailUrl);
}
...
function createPdfOnlyRecord(fileName: string, fullText: string): CnkiRecord {
...
export function extractCnkiDetailFromFrame(): DetailFrameResult {
  const cleanText = (value: string | null | undefined) => (value ?? "").replace(/\s+/g, " ").trim();
  const isUsableFrameAbstract = (value: string | null | undefined) => {
    const text = cleanText(value);
    if (!text) return false;
    return !/^(正文快照|正文摘录|正文|内容快照)[:：\s]/.test(text);
  };
  const cleanFrameAbstract = (value: string | null | undefined) => {
    const text = cleanText(value);
    return isUsableFrameAbstract(text) ? text : "";
  };
  ...
  const abstract = cleanFrameAbstract(removeLabel(
...
export function App() {
  ...
  const collectedRecords = project.records.filter(isCollectedRecord);
  const abstractCount = collectedRecords.filter((record) => isUsableAbstract(record.abstract)).length;
  const missingAbstractCount = collectedRecords.filter((record) => !isUsableAbstract(record.abstract)).length + project.failures.length;
  const fullTextCount = project.records.filter((record) => Boolean(record.fullText)).length;
  const hasCollectedRecords = collectedRecords.length > 0;
  ...
  const accountStateText = auth ? auth.user.email : "未登录";
  ...
  function openAuthView(mode: AuthMode = "login") {
    setAuthMode(mode);
    setPopupView(mode);
    setStatus(mode === "login" ? "请登录账号" : "请注册账号");
  }
  ...
  async function collectAndCompleteCurrentPage() {
    ...
    if (collectedRecords.length >= QUICK_REVIEW_MAX_PAPERS) {
    ...
    try {
      ...
      if (pending.length === 0) {
        ...
        setStatus(wasLimited
          ? `本页超过 ${QUICK_REVIEW_MAX_PAPERS} 篇，已自动停止采集`
          : "采集完成，未发现可补全链接");
        return;
      }
  ...
  async function createQuickReview() {
    ...
    try {
      setStatus("正在生成快速综述");
      const response = await generateQuickReview(auth.token, collectedRecords);
      const nextAuth = {
        ...auth,
        user: {
          ...auth.user,
  ...
  async function importFullTextPdfs(files: FileList | null) {
    ...
    if (!auth) {
      setStatus("请先登录后导入 PDF");
      openAuthView("login");
      return;
    }

    setStatus(`正在导入 ${selectedFiles.length} 个 PDF`);
    const importedRecords: CnkiRecord[] = [];
    let failed = 0;
    ...
    for (const file of selectedFiles) {
      try {
        ...
        importedRecords.push(createPdfOnlyRecord(file.name, fullText));
      } catch {
        failed += 1;
      }
    }
    ...
    if (importedRecords.length > 0) {
      await sendRuntimeMessage({ type: "SAVE_RECORDS", records: importedRecords });
    }
    if (fileInputRef.current) fileInputRef.current.value = "";
    await refresh();
    setStatus(`已导入 ${importedRecords.length} 篇 PDF 文献，失败 ${failed} 个`);
  }
  ...
  if (!auth && popupView !== "main") {
    const isRegisterView = popupView === "register";
    const authStatusText = status === "请登录账号" || status === "请注册账号" ? "" : status;

    return (
      <main className="popup-shell auth-view-shell">
        <header className="auth-view-header">
          <button type="button" className="ghost-button" onClick={returnToMainView}>返回</button>
          <h1>{isRegisterView ? "注册账号" : "登录账号"}</h1>
          <span aria-hidden="true" className="header-spacer" />
        </header>

        <section className="panel auth-screen-panel" aria-label={isRegisterView ? "注册账号" : "登录账号"}>
          {isRegisterView ? (
          ...
          <button type="button" className="auth-submit" onClick={() => handleAuth(authMode)}>
            {isRegisterView ? "注册并登录" : "登录"}
          </button>

          {authStatusText ? <p className="auth-status">{authStatusText}</p> : null}

          <div className="auth-switch-row center">
            <span>{isRegisterView ? "已有账号？" : "还没有账号？"}</span>
            <button
              type="button"
              className="text-button"
              onClick={() => switchAuthMode(isRegisterView ? "login" : "register")}
            >
              {isRegisterView ? "去登录" : "去注册"}
            </button>
          </div>
        </section>
      </main>
  ...
  return (
    <main className="popup-shell">
      <header className="app-header">
        ...
        {auth ? (
          <div className="account-actions" aria-label="当前账号">
            <span className="account-email" title={auth.user.email}>{accountStateText}</span>
            <button type="button" className="secondary account-logout" onClick={handleLogout}>退出</button>
          </div>
        ) : (
          <button type="button" className="account-state" onClick={() => openAuthView("login")}>{accountStateText}</button>
        )}
      </header>
      ...
      <section className="stats-grid" aria-label="采集统计">
        <div>
          <span>已采集</span>
          <strong>{collectedRecords.length} 篇</strong>
        </div>
        <div>
          <span>已补摘要</span>
      ...
      <section className="panel workflow-panel" aria-label="主要操作">
        <div className="panel-heading">
          <span>文献流程</span>
          <strong>{collectedRecords.length}/{QUICK_REVIEW_MAX_PAPERS}</strong>
        </div>

        <div className="workflow-card collect-card">
          <div className="workflow-copy">
            <span>采集</span>
          </div>
          <button type="button" onClick={collectAndCompleteCurrentPage} disabled={!auth || isCollecting}>
            {auth ? "采集当前页文献" : "登录后采集"}
          </button>
        </div>
        ...
        <div className="workflow-grid">
          <div className="workflow-card review-card">
            <div className="workflow-copy">
              <span>快速</span>
              {auth ? <strong>{auth.user.quickReviewQuota} 次</strong> : null}
            </div>
            <div className="button-stack">
              <button
                type="button"
                onClick={createQuickReview}
                disabled={!hasCollectedRecords || !auth || auth.user.quickReviewQuota < 1}
              >
                {auth ? "生成快速综述" : "登录后生成"}
              </button>
              <button
          ...
          <div className="workflow-card review-card">
            <div className="workflow-copy">
              <span>深度</span>
              {auth ? <strong>{auth.user.deepReviewQuota} 次</strong> : null}
            </div>
            <div className="button-stack">
              <button type="button" onClick={() => fileInputRef.current?.click()} disabled={!auth}>
                {auth ? "导入PDF" : "登录后导入"}
              </button>
              <button
                type="button"
                onClick={createDeepReview}
                disabled={!auth || fullTextCount === 0 || auth.user.deepReviewQuota < 1}
              >
                {auth ? "生成深度综述" : "登录后生成"}
              </button>
              <button
